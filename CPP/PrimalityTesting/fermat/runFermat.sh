...
for i in $(seq 0 $numTests); do
	...
	./$executable <../in/test$i.in >../out/test$i\_fermat.out
    diff -Z ../out/test$i\_fermat.out ../ref/test$i.ref > /dev/null
    if [ $? -eq 0 ]; then
		echo "Test $i ......................... perfect"
		sum=$(($sum + 1))
	else
...
./$executable <../in/bonus.in >../out/bonus_fermat.out
diff -Z ../out/bonus\_fermat.out ../ref/bonus.ref > /dev/null
if [ $? -eq 0 ]; then
